...
Green =  Wiper  of Wind direction vane - connect to A0.  Use a 1 ... 10 ÂµF / 16v capacitor between A0 and ground (observe C polarity) to avoid jitter
...
// Wind Direction
  ...
  Direction = Direction + DirCorr + 3;   // Correct for offset & 5Â° precision

 convert:       // Convert to 360Â°
  ...
  lcd.setCursor(4, 0);                // print the value from the potmeter
      ...
      lcd.print(((Direction/5)*5), DEC);    // 5Â° precision is enough to print te direction value
